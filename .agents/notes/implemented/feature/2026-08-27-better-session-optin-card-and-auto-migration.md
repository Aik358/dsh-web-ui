...
- **Card** under Settings → Web 插件 ("Better Session", order 145, slot id `better-session`): declares the third-party origin on-card ([morlay/better-session](https://github.com/morlay/better-session), MIT), shows both stores' live counts and the current posture, and gates enable/disable behind explicit confirm dialogs whose copy names every consequence from the README's trade-off list.
...
- No new package joins the cohort; an earlier same-day draft had carried this surface as a standalone package before collapsing into dsh-perf.
