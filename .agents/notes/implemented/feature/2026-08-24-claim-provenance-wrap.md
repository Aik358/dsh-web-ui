...
- `src/host-runner.ts` exports `promptText(task)`: a continuation card (a task with a `freeze` snapshot) has its instruction mandatorily wrapped in a source declaration — freeze instant (ISO), source session (`freeze.frozenBy`, "未记录" when absent), and an unreviewed-content warning — between explicit 开始/结束 markers. The wrap composes with the T4 handover preamble instead of conflicting: the reference preamble comes first, the provenance wrap then encloses the instruction. Plain tasks (no freeze) keep the bare preamble + prompt; the template is board-owned, so card text cannot remove or forge the wrap.
