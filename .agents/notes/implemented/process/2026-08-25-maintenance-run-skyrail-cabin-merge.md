...
- PRs #1100, #1093, #1070, #1031: no comments posted. Existing changes-requested reviews stand; authors have not pushed or replied. Upstream dsh-fulltext-search has since shipped the security fix (server-side session cwd only, security-test.mjs, CI matrix, v0.1.1), but the PR evidence has not been updated. dsh-archive-button still uses the rejected host-serve string-patch architecture.
...
dev now ships the Skyrail Cabin skin and the OUO Neko pet (both squash-merged, 42def344 and 93ae86d3); the Skin Center client gained the data-dsh-part="new-session" contract and its semantic adapter coverage. The community plugin index remains unpublished for the four pending applications until author evidence lands. The skill default-scope configuration was corrected from the non-existent zhu1090093659/dsh-web-ui to zhu1090093659/dsh-web (user configuration, outside this repository).