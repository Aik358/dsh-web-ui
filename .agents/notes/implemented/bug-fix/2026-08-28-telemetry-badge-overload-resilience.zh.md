...
- 新增 `badge_cache` D1 表（迁移 `0004_badge_cache.sql`）以单行存放预算计数。cron 触发器（`market/worker/wrangler.jsonc` 的 `*/30 * * * *`，`market/worker/src/index.js` 的 `scheduled` 处理器）每 30 分钟把心跳去重访客数重算进该表。
...
- 公开契约文本同步了同样的事实：docs/telemetry.md（徽章条目与客户端补报段落）、api-doc.js 端点表（徽章预算、事件 503）与 OpenAPI 描述。

## Testing

本地 `wrangler dev --test-scheduled` 加本地 D1：cron 触发把被故意改坏的行值刷回真实计数；徽章约 10 ms 返回行值；删除该行后下一请求经全表扫描 bootstrap 并重新播种；清空缓存后删表返回 200 "unavailable" JSON；删表状态 POST 返回 503 JSON。部署后生产 `wrangler tail` 显示 `Shields.io/080e177` 抓取器以 "ok" 结果、远低于 3.5 秒超时完成，shields 徽章渲染出真实计数。
