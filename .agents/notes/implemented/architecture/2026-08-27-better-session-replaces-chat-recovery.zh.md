...
- 接入遵循 `dsh-better-sidebar` 与 `@mlgbnb/dsh-archive-manager` 的既有外部集成模式：消费上游 **npm 包**（`@morlay/better-session@0.0.10`，registry 可读，SDK 版本批次与 rc.2 兼容），在 `packages/dsh-web-all/package.json` 固定版本；源码不入库。
