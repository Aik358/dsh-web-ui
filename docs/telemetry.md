...
发送是 fire-and-forget：网络不可达时静默失败，下次挂载或次日自动补报一次；标记位只在服务端接受后才写入，离线浏览器不会因此漏计整天。服务端 D1 过载时上报返回 503（客户端视为未接受，下次挂载补报），不会以 Worker 异常页响应。隐私模式等存储不可用的环境下不发任何请求。事件保留 400 天，过期由汇总读取时机会式清理。
...
- `GET /api/telemetry/badge/users` — 心跳全量去重实例数（用户数），数据随插件发版后增长。计数是全表扫描，响应在边缘缓存 30 分钟；D1 过载或不可用时回退最近一次成功计数（缓存保留 1 天），端点始终向 shields 返回合法的 200 JSON，README 徽章因此不会渲染成 inaccessible
