...
description: Safely select and install a compatible official @deepseek-ai SDK release for dsh plugin projects (dsh-web, dsh-trading, and similar monorepos) from npm using an isolated worktree, explicit cohort review, CI-equivalent validation, and controlled rollout — including post-upgrade acceptance of the DSH host CLI against every local profile (cohort skew / shadow-copy detection). Use for SDK dependency upgrades, official version checks, release-channel decisions, runtime/repository/profile cohort alignment, and host-upgrade acceptance. After the target is approved and installed, use dsh-web-sdk-compatibility for consumer adaptation, durable contracts, fixes, or new-feature adoption.
