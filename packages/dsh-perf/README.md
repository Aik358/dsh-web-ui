...
3. **Down-load and forensics**: an assistant-step shadow that stays pixel-identical to official renderers (it forwards the official renderer unchanged; only the highlight finalization of heavy messages >20KB is deferred off the turn-end hot path - no collapsing, no buttons); `content-visibility:auto` on message rows approximates virtualization (selector syntax fixed and now active independently of the HUD); a session-tail integrity observer listens on turn-end edges in the real GUI to verify the final message `finalNode`, the window tail vs. host `history` tail seq, and editor residue (the busy-state Stop click keeps the draft), recording findings into a localStorage ring buffer plus console.warn - evidence for "finished but the tail is not shown" cases; agent idle badges.
