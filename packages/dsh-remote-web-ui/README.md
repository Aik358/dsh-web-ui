...
- **Phone side**: scanning the QR binds the phone with a one-time, time-limited token and reloads into the **official Web GUI** — there is no second interface to drift from. While the phone is in portrait, the plugin injects a touch-adaptation layer over the running UI (see "The mobile adaptation layer" below). The link carries the harness browser-auth token as well, so a device that has never seen this server authenticates and pairs in one navigation.
...
- **Security**: one active one-time token (a refresh invalidates the old link; an accepted token cannot be reused; tokens expire). 停止 revokes every paired device and the current token — paired devices are cut off on their next request. Pairing is this plugin's access control for the `/remote` channel; loopback (127.0.0.1) keeps using `/api` directly. A paired device is a **full-control credential** (see Security model).
...
3. Scan with the phone (or open the copied link): the device clears the harness browser-auth gate and the pairing gate in one navigation, reloads, and boots the **official Web GUI**. On a phone, the portrait adaptation layer is already active — same layout as the desktop, same live state.
...
- **`ctx.connection.authenticatedUrl()`** (packages/client/connection): the sanctioned launch-token seam the `/pair-accept` redirect uses so LAN devices clear the browser-auth gate.
...
3. In a second tab under mobile emulation (390x844, touch) open that link: the chain `/pair-accept → /?token=<launch> → /` sets both cookies and boots the official UI — `document.body.classList` carries `dsh-remote-portrait`, the adaptation stylesheet and the whale button exist, and `__DSH_TRANSPORT__.ownsHost` is `true`. The settings surface renders host data (host mode), not a memory mirror.
