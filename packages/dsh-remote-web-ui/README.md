...
- Sibling host routes outside `/api` (`/pet/*`, `/git/*`) can consult this
  plugin's `remoteWebUiPairing` service: a live paired-device cookie is an
  allow path, `stop()` still cuts them off, and the service is absent when
