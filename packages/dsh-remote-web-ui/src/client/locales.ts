...
export const zh = {
  ...
  'settings.lanBindHint': '开启后插件把绑定默认改写为 0.0.0.0 并写入 profile 补丁（显式 --host 仍优先），热加载无需重启，同时维护主机防火墙放行（Windows/Linux；macOS 无需管理）；关闭回退 127.0.0.1。',
  'lan.cardTitle': '局域网访问',
...
export const en = {
  ...
  'settings.lanBindHint': 'When on, the plugin writes a managed block into the profile patch defaulting the bind to 0.0.0.0 (an explicit --host flag still wins); the patch hot-reloads without a restart, and the matching host firewall rule is maintained (Windows/Linux; other platforms need none). When off, the block pins 127.0.0.1.',
