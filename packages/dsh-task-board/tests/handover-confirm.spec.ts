...
describe('confirmation gate: store normalization', () => {
  it('drops a malformed handover without dropping the task row', () => {
    const row = JSON.stringify([{ ...createTask({ title: 't', description: '', prompt: 'p' }, NOW, 'id-1'), handover: { references: 'not-an-array' }, permissionConfirmedAt: 'nope' }])
...
function makeService(ledger: HostTaskLedger, now: () => number) {
  const create = vi.fn(async (request) => ok(request, { sessionId: 'session-1' }))
  ...
  const api = {
    ...
    sessions: {
      create,
      rename: async (request: { rpcId: unknown }) => ok(request, { title: 't', seq: 1 }),
      prompt,
    },
  } as unknown as ApiProxy
  const service = new TaskBoardHostService(api, { ledger, power: new PowerInhibitor({ platform: 'linux' }), now })
...
describe('handover bundle: runner override and prompt preamble', () => {
  function apiCapture() {
    ...
    const api = {
      workspace: { list: async (request: { rpcId: unknown }) => ok(request, { items: [{ workspaceId: 'ws-legacy' }, { workspaceId: 'ws-1' }] }) },
      agentPresets: { list: async (request: { rpcId: unknown }) => ok(request, { presets: [{ id: 'preset-legacy' }, { id: 'preset-a' }] }) },
      sessions: {
        create: async (request: { rpcId: unknown; payload: unknown }) => { calls.create = request.payload; return ok(request, { sessionId: 'session-1' }) },
        rename: async (request: { rpcId: unknown }) => ok(request, { title: 't', seq: 1 }),
        prompt: async (request: { rpcId: unknown; payload: unknown }) => { calls.prompt = request.payload; return ok(request, { accepted: true }) },
      },
    }
  ...
  it('the bundle triplet overrides the legacy pins and the prompt carries the reference preamble', async () => {
    ...
    const runnerWithPermission = new HostExecutionRunner(api, {
